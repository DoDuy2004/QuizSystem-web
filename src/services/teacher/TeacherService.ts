...
class TeacherService {
  ...
  getTeachers = (searchText?: string) => {
    return new Promise((resolve, reject) => {
      const params = searchText && searchText.length >= 3 ? { searchText } : {};

      axios
        .get(`${import.meta.env.VITE_DOMAIN}/api/teachers`, { params })
        .then((response) => resolve(response))
        .catch(function (error) {
          if (error.response) {
            // The request was made and the server responded with a status code
  ...
  getTeacher = (id: string) => {
    return new Promise((resolve, reject) => {
      axios
        .get(`${import.meta.env.VITE_DOMAIN}/api/teachers/${id}`)
        .then((response) => resolve(response))
        .catch(function (error) {
          if (error.response) {
            // The request was made and the server responded with a status code
  ...
  createTeacher = (params: any) => {
    return new Promise((resolve, reject) => {
      const form = params?.form;
      axios
        .post(`${import.meta.env.VITE_DOMAIN}/api/teachers`, form)
        .then((response) => resolve(response))
        .catch(function (error) {
          if (error.response) {
            // The request was made and the server responded with a status code
  ...
  updateTeacher = (params: any) => {
    return new Promise((resolve, reject) => {
      ...
      const form = params?.form;
      axios
        .put(`${import.meta.env.VITE_DOMAIN}/api/teachers/${id}`, form)
        .then((response) => resolve(response))
        .catch(function (error) {
          if (error.response) {
            // The request was made and the server responded with a status code
