...
class QuestionBankService {
  ...
  getQuestionBanks = (searchText?: string) => {
    return new Promise((resolve, reject) => {
      const params = searchText && searchText.length >= 3 ? { searchText } : {};

      axios
        .get(`${import.meta.env.VITE_DOMAIN}/api/questionbanks`, { params })
        .then((response) => resolve(response))
        .catch(function (error) {
          if (error.response) {
            // The request was made and the server responded with a status code
  ...
  getQuestions = () => {
    return new Promise((resolve, reject) => {
      axios
        .get(`${import.meta.env.VITE_DOMAIN}/api/questions`)
        .then((response) => resolve(response))
        .catch(function (error) {
          if (error.response) {
            // The request was made and the server responded with a status code
    ...
    return new Promise((resolve, reject) => {
      ...
      axios
        .post(
          ...
          }/api/questionbanks/importquestionsfile-preview`,
          formData,
          {
            headers: {
